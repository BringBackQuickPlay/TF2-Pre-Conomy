...
static ConVar sv_tf2pc_keyvalues_transport(
	...
	"Accept TF2PC's generic serialized-KeyValues command transport. The client cvar must also be enabled."
);

static ConVar sv_tf2pc_keyvalues_transport_debug(
	"sv_tf2pc_keyvalues_transport_debug",
	"0",
	FCVAR_GAMEDLL | FCVAR_CHEAT,
	"Print non-sensitive TF2PC KeyValues transport stages to the server console."
);

// Permanent-release alternative:
// Uncomment this definition to force the workaround on in the binary. When
// forced, server owners cannot disable it through a cvar. Leave it commented
// while testing or while waiting for Valve to repair ServerCmdKeyValues().
// #define TF2PC_FORCE_SERVER_KEYVALUES_COMMAND_TRANSPORT 1

#ifndef TF2PC_FORCE_SERVER_KEYVALUES_COMMAND_TRANSPORT
#define TF2PC_FORCE_SERVER_KEYVALUES_COMMAND_TRANSPORT 0
#endif

#define TF2PC_SERVER_KV_TRANSPORT_DEBUG( ... ) \
	do \
	{ \
		if ( sv_tf2pc_keyvalues_transport_debug.GetBool() ) \
		{ \
			Msg( __VA_ARGS__ ); \
		} \
	} while ( false )
...
{
	return
		TF2PC_FORCE_SERVER_KEYVALUES_COMMAND_TRANSPORT != 0 ||
		sv_tf2pc_keyvalues_transport.GetBool();
}
...
{
	if ( pState && sv_tf2pc_keyvalues_transport_debug.GetBool() )
	{
