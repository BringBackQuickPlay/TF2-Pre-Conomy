...
private:
	...
	struct FriendlySentryMemory
	{
		...
		CHandle< CTFPlayer > builder;
		Vector lastKnownPosition;
		CountdownTimer forgetTimer;
	};

	FriendlySentryMemory m_friendlySentryMemory[2];
	...
	bool ShouldConsiderFriendlyEngineerActions( CTFBot *me ) const;
	void ClearExpiredFriendlySentryMemory( void );
	void RememberFriendlySentry( CTFBot *me, CObjectSentrygun *sentry );
	void UpdateFriendlySentryMemory( CTFBot *me );
