...
//   0 = Valve's original ServerCmdKeyValues path
//   1 = TF2PC's generic serialized-KeyValues command transport
//   2 = TF2PC's transport plus non-sensitive client console debugging
...
// Uncomment this definition to force mode 1 in the binary. Users then cannot
// switch back to Valve's path through the cvar. Use 2 instead only when a
// permanently verbose diagnostic build is specifically desired.
// #define TF2PC_FORCE_CLIENT_KEYVALUES_COMMAND_TRANSPORT_MODE 1

#ifndef TF2PC_FORCE_CLIENT_KEYVALUES_COMMAND_TRANSPORT_MODE
#define TF2PC_FORCE_CLIENT_KEYVALUES_COMMAND_TRANSPORT_MODE 0
#endif

static int TF2PCClientKeyValuesTransportMode()
{
	if ( TF2PC_FORCE_CLIENT_KEYVALUES_COMMAND_TRANSPORT_MODE != 0 )
		return TF2PC_FORCE_CLIENT_KEYVALUES_COMMAND_TRANSPORT_MODE;

	return cc_tf2pc_keyvalues_transport.GetInt();
}

static bool TF2PCClientKeyValuesTransportEnabled()
{
	return
		TF2PCClientKeyValuesTransportMode() >=
		k_eTF2PCKeyValuesTransportMode_Enabled;
}

static bool TF2PCClientKeyValuesTransportDebugEnabled()
{
	return
		TF2PCClientKeyValuesTransportMode() >=
...
#define TF2PC_CLIENT_KV_TRANSPORT_DEBUG( ... ) \
	do \
	{ \
		if ( TF2PCClientKeyValuesTransportDebugEnabled() ) \
		{ \
			Msg( __VA_ARGS__ ); \
		} \
	} while ( false )
